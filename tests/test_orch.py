...
from app.models import (Datasource, Environment, Job, MetricSample,
                        ProtoPackage)
...
def test_orchestrator(db):
    # 环境已改为 DB 管理：dev 环境 masters 必须指向 mock SeaTunnel（upsert）
    ...
    ds = Datasource(env="dev", name="k", type="kafka", connection={"servers": "k:9092"},
                    metadata_status="ok")
    db.add_all([pkg, ds])
    db.commit()
    ...
    job2 = Job(name="orch_test_dup", env="dev", biz_line="md", source_type="kafka",
               datasource_id=ds.id,
