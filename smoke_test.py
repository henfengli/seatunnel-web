# -*- coding: utf-8 -*-
"""冒烟测试：proto 解析 + 字段映射 + 渲染管线（不依赖外部服务）。"""
import sys

sys.path.insert(0, ".")

from app.services import proto_center, field_mapping, render  # noqa: E402
...
from app.services.metadata import mongo_d  # noqa: E402
...
pkg_e = ProtoPackage(name="t_enum", content=ENUM_PROTO,
                     parsed_json=ProtoPackage._dumps(proto_center.parse_proto(ENUM_PROTO)))
...
# 3. schema_fields_for：嵌套 message → HOCON row，repeated message → array<{...}>
init_db()
db = SessionLocal()
# 环境已改为 DB 管理：插入 dev 环境（渲染管线 envs.get_env 依赖它）
if not db.query(Environment).filter_by(name="dev").first():
    db.add(Environment(name="dev", seatunnel_masters="http://127.0.0.1:8080",
...
# 幂等：清掉上次运行的种子残留（否则 UNIQUE 约束冲突）
db.query(Job).filter(Job.name.in_(["smoke_kafka_job", "smoke_sasl_job"])).delete(synchronize_session=False)
db.query(Datasource).filter(Datasource.name.in_(["k1", "k_sasl"])).delete(synchronize_session=False)
db.query(ProtoPackage).filter(ProtoPackage.name.in_(["t_nested", "t_flat"])).delete(synchronize_session=False)
db.commit()
pkg = ProtoPackage(name="t_nested", content=NESTED_PROTO, parsed_json=ProtoPackage._dumps(p2))
db.add(pkg)
db.commit()
fields = proto_center.schema_fields_for(pkg, "Event")
fmap = {f["name"]: f["st_type"] for f in fields}
print("  Event schema fields:", fmap)
...
ds = Datasource(env="dev", name="k1", type="kafka", connection_json='{"servers": "kafka1:9092"}')
db.add(ds)
db.commit()
job = Job(
...
ds_sasl = Datasource(env="dev", name="k_sasl", type="kafka", connection_json=Datasource._dumps({
    "servers": "kafka1:9093",
...
# PLAINTEXT（第 5 步的 conf）不应输出任何 SASL 配置
...
ts_mapping = field_mapping.append_timestamp_columns(
    field_mapping.build_mapping("kafka", fields, variant_enabled=True), "kafka")
...
from app.services import doris_ddl  # noqa: E402
...
job.field_mapping_json = Job._dumps(flat_mapping)
...
job.field_mapping_json = Job._dumps(flat_mapping[:2])  # 去掉两个时间戳列（保留拍平项）
...
# 幂等：先清上次残留
db.query(Job).filter(Job.name.in_(["smoke_mg_job", "smoke_mg_cdc_job"])).delete(synchronize_session=False)
db.query(Datasource).filter(Datasource.name == "mg_smoke").delete(synchronize_session=False)
db.commit()
mg_ds = Datasource(env="dev", name="mg_smoke", type="mongodb",
                   connection_json='{"host": "mg:27017"}')
db.add(mg_ds)
db.commit()
mg_job = Job(name="smoke_mg_job", env="dev", biz_line="db1", source_type="mongodb",
             ...
             field_mapping_json=Job._dumps(mg_mapping), options_json="{}")
...
cdc_job = Job(name="smoke_mg_cdc_job", env="dev", biz_line="db1", source_type="mongodb",
              ...
              field_mapping_json=Job._dumps(mg_mapping),
              options_json='{"mongo_mode": "cdc", "cdc_startup_mode": "latest"}')
...
from app.templating import _mask_conf  # noqa: E402
...
from app.core.crypto import sanitize_error  # noqa: E402
from app.services.metadata import mongo_d  # noqa: E402
from app.api.pages import _build_connection, _collect_options, _parse_mapping_form  # noqa: E402
...
conn, err = _build_connection("postgresql", FormData(
...
m2, err = _parse_mapping_form(_map_form2())
...
opts, err = _collect_options(FormData([("table_model", "AGGREGATE")]),
