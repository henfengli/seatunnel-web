...
运行: .venv/Scripts/python.exe recreate_test.py（在 seatunnel-web/ 目录下）
"""
import json
import re
import sys
...
from app.services import doris_ddl, orchestrator  # noqa: E402
...
# 7. 提交/更新预检拒绝（mock SeaTunnel，验证作业状态不被破坏）
...
if ds is None:
    ds = Datasource(env="dev", name="rc", type="kafka", connection_json='{"servers": "k:9092"}')
    db.add(ds)
db.query(Job).filter(Job.name == "rc_test").delete(synchronize_session=False)
db.commit()
...
db.query(Job).filter(Job.name == "rc_test").delete(synchronize_session=False)
db.query(Datasource).filter_by(env="dev", name="rc").delete(synchronize_session=False)
db.commit()
db.close()
server.shutdown()
print("RECREATE", "OK" if ok else "FAILED")
