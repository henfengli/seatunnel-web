...
运行: .venv/Scripts/python.exe batch_test.py（在 seatunnel-web/ 目录下）
"""
import json
import sys
import threading
...
from app.models import BatchTask, Datasource, Environment, Job, ProtoPackage  # noqa: E402
from app.services import proto_center  # noqa: E402
from app.services.field_mapping import append_timestamp_columns, build_mapping  # noqa: E402
...
# 绕过 Doris DDL（与 orch_test 同款 mock）
...
db.add(env)
ds = Datasource(
    env="dev", name="bk", type="kafka", connection_json='{"servers": "k:9092"}',
    metadata_status="ok",
    metadata_json=Datasource._dumps({
...
_ids = [r[0] for r in db.query(Job.id).filter(Job.name.in_(["IPP", "POS", "ORDER"])).all()]
from app.models import JobVersion, JobEvent, MetricSample  # noqa: E402

db.query(JobVersion).filter(JobVersion.job_id.in_(_ids or [-1])).delete(synchronize_session=False)
db.query(JobEvent).filter(JobEvent.job_id.in_(_ids or [-1])).delete(synchronize_session=False)
db.query(MetricSample).filter(MetricSample.job_id.in_(_ids or [-1])).delete(synchronize_session=False)
db.query(Job).filter(Job.name.in_(["IPP", "POS", "ORDER"])).delete(synchronize_session=False)
db.query(Datasource).filter_by(env="dev", name="bk").delete(synchronize_session=False)
db.query(ProtoPackage).filter_by(name="btick").delete(synchronize_session=False)
db.commit()
db.add_all([ds, pkg])
db.commit()
...
# 清理（保证可重复运行；BatchItem 无 bulk-delete 级联，先删）
from app.models import BatchItem  # noqa: E402
...
db.close()
server.shutdown()
print("BATCH", "OK" if ok else "FAILED")
